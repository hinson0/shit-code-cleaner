...
from reference_impl.files import ToolError, read_file
...
def test_read_file_returns_content(tmp_path: Path) -> None:
    ...
    assert read_file(tmp_path, "demo.py") == "print('hello')\n"
...
def test_read_file_rejects_path_outside_repo(tmp_path: Path) -> None:
    ...
    with pytest.raises(ToolError, match="escapes repository"):
